...
Env overrides:
  ...
  PIPER_DOCKER_CONTAINER default: wyoming-piper (run piper via docker exec into this container)
...
PIPER_DOCKER_CONTAINER = os.environ.get("PIPER_DOCKER_CONTAINER", "wyoming-piper").strip()
...
def _synthesize_with_piper(text: str) -> Tuple[np.ndarray, int]:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(f"Piper command not found (bin={PIPER_BIN})" if not PIPER_DOCKER_CONTAINER else "Docker CLI not found for Piper docker-exec mode"),
        )
